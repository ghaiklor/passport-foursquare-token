...
var FoursquareTokenStrategy = require('../');
var fakeProfile = JSON.stringify(require('./fixtures/profile.json'));

describe('FoursquareTokenStrategy:init', function () {
  it('Should properly export Strategy constructor', function () {
    assert.equal(typeof FoursquareTokenStrategy, 'function');
    assert.equal(typeof FoursquareTokenStrategy.Strategy, 'function');
    assert.equal(FoursquareTokenStrategy, FoursquareTokenStrategy.Strategy);
  });

  it('Should properly initialize', function () {
    var strategy = new FoursquareTokenStrategy({
      clientID: '123',
      clientSecret: '123'
    }, function () {
    });
...
describe('FoursquareTokenStrategy:authenticate', function () {
  describe('Authenticate without passReqToCallback', function () {
    var strategy;

    before(function () {
      strategy = new FoursquareTokenStrategy({
        clientID: '123',
        clientSecret: '123'
...
describe('FoursquareTokenStrategy:userProfile', function () {
